...
# colors
. ./vendor/axxion/tools/colors

if [ ! -d ".repo" ]; then
    echo -e ${red}"No .repo directory found.  Is this an Android build tree?"${txtrst}
    exit 1
fi
if [ ! -d "vendor/axxion" ]; then
    echo -e ${red}"No vendor/axxion directory found.  Is this a PAC build tree?"${txtrst}
    exit 1
fi
...
# get current version
eval $(grep "^AXXION_VERSION_" vendor/axxion/config/common.mk | sed 's/ [:=]\+ /=/g' | sed 's/shell //g')
VERSION="$AXXION_VERSION_MAJOR.$AXXION_VERSION_MINOR"

echo -e ${cya}"Building ${bldred}AxxionKat v$VERSION"${txtrst}

# AXXION device dependencies
echo -e ""
echo -e ${bldblu}"Looking for Axxion product dependencies${txtrst}"${cya}
if [ "$opt_kr" -ne 0 ]; then
    vendor/axxion/tools/getdependencies.py "$device" "$opt_kr"
else
    vendor/axxion/tools/getdependencies.py "$device"
fi
echo -e "${txtrst}"
...
if [ "$opt_fetch" -ne 0 ]; then
        ./vendor/axxion/tools/cherries.sh $device
fi
...
lunch "axxion_$device-userdebug";
...
# squisher
vendor/axxion/tools/squisher

# cleanup unused built
rm -f $OUTDIR/target/product/$device/cm-*.*
rm -f $OUTDIR/target/product/$device/axxion_*-ota*.zip
