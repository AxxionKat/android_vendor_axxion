...
# get current version
eval $(grep "^AXXION_VERSION_" vendor/axxion/config/common.mk | sed 's/ *//g')
VERSION="AXXION-v$(AXXION_VERSION_MAJOR).$(AXXION_VERSION_MINOR)-$(AXXION_VERSION_STATE)"

echo -e ${cya}"Building ${ppl}AxxionKat ${bldylw}$VERSION"${txtrst}
...
make -j"$opt_jobs" showcommands axxion
else
make -j"$opt_jobs" axxion
fi
echo -e ""
